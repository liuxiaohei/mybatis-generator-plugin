...
public class LogicalDeletePlugin extends PluginAdapter {
        ...
        for (IntrospectedColumn column : columns) {
            if (column.getActualColumnName().equalsIgnoreCase(logicalDeleteColumn)) {
                ...
                if (JDBCType.BIGINT == type
                    ...
                    logger.warn("chrm(逻辑删除插件):" + introspectedTable.getFullyQualifiedTable() + "逻辑删除列(" + introspectedTable.getTableConfigurationProperty(LOGICAL_DELETE_COLUMN_KEY) + ")的类型不在支持范围（请使用数字列，字符串列，布尔列）！");
                }
            }
        }

        if (introspectedTable.getTableConfigurationProperty(LOGICAL_DELETE_COLUMN_KEY) != null && this.logicalDeleteColumn == null) {
            logger.warn("chrm(逻辑删除插件):" + introspectedTable.getFullyQualifiedTable() + "没有找到您配置的逻辑删除列(" + introspectedTable.getTableConfigurationProperty(LOGICAL_DELETE_COLUMN_KEY) + ")！");
        }
    }
    ...
    public boolean clientGenerated(Interface interfaze, TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        if (this.logicalDeleteColumn != null) {
            ...
            logger.debug("chrm(逻辑删除插件):" + interfaze.getType().getShortName() + "增加方法logicalDeleteByExample。");
            ...
            if (introspectedTable.hasPrimaryKeyColumns()) {
                ...
                logger.debug("chrm(逻辑删除插件):" + interfaze.getType().getShortName() + "增加方法logicalDeleteByPrimaryKey。");
            }
        }
        return true;
    }
    ...
    public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
        if (this.logicalDeleteColumn != null) {
            ...
            logger.debug("chrm(逻辑删除插件):" + introspectedTable.getMyBatis3XmlMapperFileName() + "增加方法logicalDeleteByExample的实现。");
            ...
            if (introspectedTable.hasPrimaryKeyColumns()) {
                ...
                logger.debug("chrm(逻辑删除插件):" + introspectedTable.getMyBatis3XmlMapperFileName() + "增加方法logicalDeleteByPrimaryKey的实现。");
            }
        }
        return true;
    }
    ...
    public boolean modelBaseRecordClassGenerated(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        if (this.logicalDeleteColumn != null) {
            ...
            fields.add(0, field);
            logger.debug("chrm(逻辑删除插件):" + topLevelClass.getType().getShortName() + "增加方法DEL_FLAG的常量。");
        }
        return true;
    }
        ...
        if (this.logicalDeleteColumn != null) {
            ...
            for (InnerClass innerClass : innerClasses) {
                if ("Criteria".equals(innerClass.getType().getShortName())) {
                    ...
                    sb.append("(").append(modelName).append(".").append(DEL_FLAG_NAME).append(")");
                    sb.append(" : ");
                    ...
                    logger.debug("chrm(逻辑删除插件):" + topLevelClass.getType().getShortName() + "." + innerClass.getType().getShortName() + "增加andDeleted方法。");
                }
            }
        }
        return true;
