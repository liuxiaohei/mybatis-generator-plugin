...
public class ModelBuilderPlugin extends PluginAdapter {
    ...
    public boolean modelBaseRecordClassGenerated(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        ...
        logger.debug("chrm(数据Model链式构建插件):" + topLevelClass.getType().getShortName() + "增加内部Builder类。");
        ...
        logger.debug("chrm(数据Model链式构建插件):" + topLevelClass.getType().getShortName() + ".Builder增加的构造方法。");
        ...
        if (topLevelClass.getSuperClass() != null && topLevelClass.getSuperClass().compareTo(new FullyQualifiedJavaType(introspectedTable.getPrimaryKeyType())) == 0) {
            for (IntrospectedColumn introspectedColumn : introspectedTable.getPrimaryKeyColumns()) {
                ...
                method.addBodyLine("obj." + setterMethod.getName() +
                        "(" + field.getName() + ");");
                method.addBodyLine("return this;");
                innerClass.addMethod(method);
                logger.debug("chrm(数据Model链式构建插件):" + topLevelClass.getType().getShortName() + ".Builder增加" + method.getName() + "方法(复合主键)。");
            }
        }
        ...
        for (Field field : fields) {
            ...
            method.setReturnType(innerClass.getType());
            method.addParameter(new Parameter(field.getType(), field.getName()));
            method.addBodyLine("obj." + field.getName() +
                    " = " + field.getName() + ";");
            method.addBodyLine("return this;");
            innerClass.addMethod(method);
            logger.debug("chrm(数据Model链式构建插件):" + topLevelClass.getType().getShortName() + ".Builder增加" + method.getName() + "方法。");
        }
        ...
        logger.debug("chrm(数据Model链式构建插件):" + topLevelClass.getType().getShortName() + ".Builder增加build方法。");
